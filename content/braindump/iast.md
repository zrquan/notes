...
IAST 是 AST 其中的一个类别，AST 是 Application Security Testing 的简称，在他之下衍生出来以下几种类型：
...
IAST 可以理解为 SAST(静态测试) 和 DAST(动态测试) 的结合，通过运行时代理的方法在测试阶段监控、分析应用程序的行为
...
白盒主要由源代码分析和字节码分析组成，涉及[污点分析]({{< relref "污点分析.md" >}})和符号执行等技术。
...
黑盒主要利用响应测试对外的 API。常见的方式是各种 POC 或指纹扫描，FUZZ 模糊测试技术和动态污点分析技术。
